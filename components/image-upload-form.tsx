...
export function ImageUploadForm() {
  ...
  useEffect(() => {
    if (selectedFile) {
      ...
      img.onload = () => {
        ...
        const scale = Math.max(finalWidth / img.width, finalHeight / img.height) * 1.04 // 4% Überlappung für sicheren Rand
    ...
    async (file: File): Promise<{ blob: Blob; filename: string }> => {
      return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
          ...
          // Berechne die Position so, dass das Bild den Canvas komplett ausfüllt
          const scale = Math.max(finalWidth / img.width, finalHeight / img.height) * 1.04 // 4% Überlappung
          const scaledWidth = img.width * scale
          const scaledHeight = img.height * scale
        ...
        img.onerror = () => reject(new Error("Bilddatei konnte nicht geladen werden"))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({
        ...
        type: "success",
        details: `Datei: ${filename}\nBucket: ${result.bucket}\nGröße: ${(result.size / 1024).toFixed(2)} KB`,
      })
  ...
  const handleAdditionalFilesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (largeFile) {
        setStatus({
          message: "Fehler: Eine oder mehrere Dateien sind zu groß (Maximum 10MB)",
          type: "error",
        })
        e.target.value = ""
        return
  ...
  const handleUploadAdditional = async () => {
    if (!additionalFiles || !baseName) {
      setStatus({
        message: "Bitte wählen Sie Bilder aus und geben Sie einen Basisnamen ein",
        type: "error",
      })
      return
    }
    ...
    try {
      ...
      const fileInput = document.querySelector('input[name="additional"]') as HTMLInputElement
      if (fileInput) fileInput.value = ""
    } catch (error) {
      console.error("Upload error:", error)
      setStatus({
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-4">
        ...
        {preview && (
          <div className="mt-4 relative">
            ...
            >
              <img ref={imageRef} src={preview || "/placeholder.svg"} alt="Vorschau" className="w-full h-auto" />
              {focalPoint && (
                <>
                  <div
                  ...
                  <div
                    ...
                    style={{
                      ...
                      left: "50%",
                      top: "50%",
                      transform: `translate(-50%, -50%) translateY(${(focalPoint.y - 0.5) * 100}%) translateX(${(focalPoint.x - 0.5) * 100}%)`,
                    }}
                  />
                </>
              )}
                ...
                <span className="block">
                  ...
                  {focalPoint && (
                    <span className="block text-xs">
                      Bildmitte: {Math.round(focalPoint.x * 100)}% x {Math.round(focalPoint.y * 100)}%
                    </span>
                  )}
                </span>
              )}
      ...
      <div className="space-y-6">
        <div className="border-t pt-6">
          ...
          <MarkdownEditor onContentChange={setMdxContent} initialContent={mdxContent} />
        </div>
      </div>
      ...
      <div className="border-t pt-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="additional">Bilder auswählen (max. 10MB pro Bild)</Label>
            <div className="flex gap-4">
              ...
              <Button
                type="button"
                disabled={!additionalFiles || !baseName || isUploadingAdditional}
      ...
      <div className="flex gap-4">
        ...
        {uploadedUrl && (
          <Button type="button" variant="outline" onClick={handleGithubCommit} className="flex-1">
            <Upload className="mr-2 h-4 w-4" />
            Zu GitHub committen
          </Button>
        )}
      ...
      {status.message && (
        <Alert variant={status.type === "error" ? "destructive" : "default"}>
          <AlertDescription>
            <div className="space-y-2">
              <p>{status.message}</p>
              {status.details && <pre className="text-xs whitespace-pre-wrap">{status.details}</pre>}
            </div>
          </AlertDescription>
        </Alert>
      )}
